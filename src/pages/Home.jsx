...
import { Shield, TrendingUp, Users, Award, ArrowRight, Car, CheckCircle, Search, X } from 'lucide-react'
...
export default function Home() {
  ...
  const handleBuscar = () => setBuscaAtiva(termoDigitado)
        ...
        <div className="max-w-7xl mx-auto">
          <div className="max-w-2xl">
            ...
            <div className="flex gap-3 flex-col sm:flex-row">
              <input
                type="text"
                placeholder="Buscar por seguradora, plano..."
                value={termoDigitado}
                onChange={e => setTermoDigitado(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && handleBuscar()}
                className="flex-1 px-4 py-3 rounded-xl text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 shadow-lg bg-white"
              />
              <button
                ...
                {seguradoras.map(s => (
                  <div key={s.id} className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow text-center">
                    <div className="h-12 flex items-center justify-center mb-3">
                      <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center overflow-hidden border border-gray-100 p-1.5 shadow-sm">
                        {s.logo ? (
                          <img 
...
function Building2Icon(props) {
  return <svg className={props.className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
}
function FileTextIcon(props) {
  return <svg className={props.className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
}
function ShieldIcon(props) {
  return <svg className={props.className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
}
